...
public class TNG7_priority {
    // What is priority:
    // why needs to use priority ? : If we not use priority TestNG will set the priority based on assKey (alphabetically) value. To avoid them needs to use priority.
    // how to define ? : using priority keyword
    // TestNG internally doesn't maintain the order of test case execution, so that's why prioritization is required
